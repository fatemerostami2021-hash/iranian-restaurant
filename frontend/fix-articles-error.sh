...
echo "🔧 در حال رفع خطای <p> در مدیریت مقالات..."

# پیدا کردن و جایگزینی در فایل ArticlesManagement.jsx
cat > src/admin/ArticlesManagement.jsx << 'INNEREOF'
...
function ArticleForm({ article, onSave, onCancel, isEditing }) {
  ...
  const handleImageUpload = async (e) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (file.size > 5 * 1024 * 1024) {
        alert(`حجم عکس ${i + 1} بیشتر از 5MB است`);
        continue;
      }
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!formData.slug && formData.title.fa) {
      formData.slug = formData.title.fa
        .replace(/[^\w\s]/g, '')
        .replace(/\s+/g, '-')
        .toLowerCase();
    }
      ...
      <div className="bg-gray-900 rounded-2xl p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto border border-gray-700 shadow-2xl">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-white">
            {isEditing ? '✏️ ویرایش مقاله' : '➕ افزودن مقاله جدید'}
          </h2>
          <button 
            onClick={onCancel} 
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="مثلاً: article-title"
                dir="ltr"
              />
            </div>
            <div>
          ...
          <div>
            ...
            {languages.map(({ key, label, dir }) => (
              <div key={key} className="flex items-start gap-2 mb-2">
                ...
                <textarea
                  value={formData.excerpt[key] || ''}
                  onChange={(e) => handleExcerptChange(key, e.target.value)}
                  dir={dir}
                  rows="2"
                  className="flex-1 bg-gray-800 border border-gray-700 rounded-lg p-2 text-white focus:ring-2 focus:ring-[#FFD700] focus:outline-none"
                  placeholder={`خلاصه به ${label}`}
                />
              </div>
            ))}
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">
              <FiTag className="inline mr-1" /> تگ‌ها
            </label>
            <div className="flex items-center gap-2">
              <input
                ...
                className="flex-1 bg-gray-800 border border-gray-700 rounded-lg p-2 text-white focus:ring-2 focus:ring-[#FFD700] focus:outline-none"
                placeholder="تگ جدید را وارد کنید و Enter بزنید"
              />
              <button
                type="button"
                onClick={handleAddTag}
                className="bg-[#FFD700] text-black px-4 py-2 rounded-lg hover:bg-[#FFC700] transition-colors"
              >
                افزودن
              </button>
            </div>
          ...
          <div className="flex gap-3 pt-4 border-t border-gray-700">
            ...
            >
              ...
              {isEditing ? 'ذخیره تغییرات' : 'افزودن مقاله'}
            </button>
            <button
              type="button"
              onClick={onCancel}
...
export default function ArticlesManagement() {
  ...
  const fetchArticles = async () => {
    try {
      ...
      setArticles([
        {
          _id: '1',
          title: { fa: 'آشنایی با غذاهای سنتی ایرانی', en: 'Introduction to Persian Cuisine', ar: 'مقدمة في المطبخ الإيراني' },
          slug: 'introduction-to-persian-cuisine',
          excerpt: { fa: 'غذاهای سنتی ایرانی دارای تنوع و طعم‌های بی‌نظیری هستند...', en: '', ar: '' },
          category: 'blog',
          tags: ['غذا', 'سنتی', 'ایرانی'],
          status: 'published',
          featured: true,
          images: ['https://via.placeholder.com/800x400/FFD700/000000?text=Persian+Cuisine'],
          author: 'مدیر',
  ...
  const handleSave = async (formData) => {
    try {
      if (editingArticle) {
        await axios.put(`http://localhost:5000/api/admin/articles/${editingArticle._id}`, formData, {
          headers: { Authorization: `Bearer ${token}` }
        });
      } else {
  ...
  const filteredArticles = articles.filter(a => {
    const matchesSearch = 
      a.title?.fa?.includes(search) || 
      a.title?.en?.includes(search) || 
      a.slug?.includes(search);
    ...
    <div className="text-white">
      ...
      <div className="flex flex-wrap items-center gap-4 mb-6 bg-gray-800 rounded-lg p-3">
        <div className="flex-1 min-w-[200px] flex items-center gap-2">
          ...
          <input
            type="text"
            placeholder="جستجو در عنوان یا اسلاگ مقاله..."
            value={search}
                  ...
                  <tr key={article._id} className="border-t border-gray-700 hover:bg-gray-700/30 transition-colors">
                    ...
                    <td className="p-3">
                      <div>
                        ...
                        <p className="text-xs text-gray-500 font-mono">{article.slug || '-'}</p>
                        {article.video && (
...
echo "✅ فایل ArticlesManagement.jsx با موفقیت به‌روزرسانی شد!"
echo "✅ مشکل خطای <p> برطرف شد!"
echo ""
echo "🔄 برای اعمال تغییرات، پروژه را ریستارت کنید:"
echo "   npm start"
