...
const galleryVideos = [
  {
    ...
    titleFallback: 'تجربه یک باربکیوی سلطنتی در منزل شما',
    descKey: 'homePage.videoGallery.videos.main.desc',
    descFallback: 'تیم حرفه‌ای ما با تجهیزات کامل به منزل شما می‌آیند تا کباب داغ و زنده را در حضور مهمانان شما سرو کنند. شبی متفاوت و به‌یادماندنی بسازید!',
    views: 24800,
    likes: 1920,
  },
  {
    ...
    categoryFallback: 'تجهیزات',
    titleKey: 'homePage.videoGallery.videos.tour.title',
    titleFallback: 'تجهیزات و راه‌اندازی سیار',
    descKey: 'homePage.videoGallery.videos.tour.desc',
    descFallback: 'نگاهی به چیدمان حرفه‌ای و تجهیزات کامل آشپزخانه سیار ما که با کمترین دخالت در فضای شما، بهترین خدمات را ارائه می‌دهد.',
    views: 18200,
    likes: 1430,
  },
  {
    ...
    categoryFallback: 'آشپزی زنده',
    titleKey: 'homePage.videoGallery.videos.kabab.title',
    titleFallback: 'استاد کباب در حضور شما',
    descKey: 'homePage.videoGallery.videos.kabab.desc',
    descFallback: 'تماشای هنر کباب کردن با ذغال و عطر دود ناب، دقیقاً در حیاط یا سالن پذیرایی شما. هیجان یک آشپزخانه حرفه‌ای در خانه شما!',
    views: 31400,
    likes: 2680,
  },
  {
    ...
    categoryFallback: 'مهمانی‌ها',
    titleKey: 'homePage.videoGallery.videos.moments.title',
    titleFallback: 'مهمانی‌های خاص و به‌یادماندنی',
    descKey: 'homePage.videoGallery.videos.moments.desc',
    descFallback: 'مهمانی‌ها، تولدها و جشن‌های شرکتی که با سرویس کباب داغ در منزل، به شبی متفاوت و خاطره‌انگیز تبدیل شدند.',
    views: 12600,
    likes: 980,
  },
  {
    ...
    categoryFallback: 'تیم ما',
    titleKey: 'homePage.videoGallery.videos.chef.title',
    titleFallback: 'سرآشپز و تیم متخصص',
    descKey: 'homePage.videoGallery.videos.chef.desc',
    descFallback: 'آشنایی با سرآشپزها و تیم پشتیبان ما که با لباس فرم و رعایت کامل بهداشت، به منزل شما اعزام می‌شوند.',
    views: 9800,
    likes: 720,
  },
  {
    ...
    categoryKey: 'homePage.videoGallery.categories.catering',
    categoryFallback: 'سرویس منزل',
    titleKey: 'homePage.videoGallery.videos.catering.title',
    titleFallback: 'کباب داغ در منزل شما',
    descKey: 'homePage.videoGallery.videos.catering.desc',
    descFallback: 'فرقی نمی‌کند مهمانی شما در خانه، باغ یا سینما باشد؛ ما کباب داغ و سلطنتی را با سرویس VIP برای شما ارسال و سرو می‌کنیم.',
    views: 15700,
    likes: 1180,
  },
];
...
function MainVideoPlayer({ video, isDark, isLiked, onLike, t }) {
  ...
  // پس‌زمینه نوار کنترل پررنگ‌تر شد تا دکمه‌ها کاملاً پیدا باشند
  const controlsBg = 'bg-black/90 border-t border-white/10';
  ...
  return (
    <motion.div
      ...
      // عرض ویدیو به max-w-[1400px] افزایش یافت
      className={`relative w-full max-w-[1400px] mx-auto rounded-3xl overflow-hidden ${
    ...
    >
      ...
      >
        ...
        <div
          className={`absolute inset-x-0 bottom-10 md:bottom-14 z-20 px-6 md:px-10 transition-opacity duration-300 ${
        ...
        >
          <div className="pointer-events-auto">
            ...
            <motion.h3
              ...
              className="font-['Vazirmatn'] text-xl sm:text-2xl md:text-4xl lg:text-5xl font-black text-white leading-tight drop-shadow-lg"
            >
            ...
            <motion.p
              ...
              className="font-['Vazirmatn'] text-xs sm:text-sm md:text-lg text-gray-200 mt-2 max-w-3xl leading-relaxed drop-shadow-lg"
            >
        ...
        {/* نوار کنترل با ابعاد بزرگتر و وضوح بالاتر */}
        <div
          className={`absolute inset-x-0 bottom-0 z-20 ${controlsBg} backdrop-blur-md transition-opacity duration-300 ${
            showControls || !isPlaying ? 'opacity-100' : 'opacity-0'
          }`}
        >
          <div className="h-1.5 md:h-2 bg-white/20 cursor-pointer relative" onClick={handleProgressClick}>
          ...
          <div className="px-4 md:px-6 py-3 flex items-center justify-between gap-2">
            <div className="flex items-center gap-3 md:gap-4">
              <button onClick={togglePlay} className="text-white hover:text-[#FFD700] hover:scale-110 transition-all p-1">
                {isPlaying ? (
                  <svg className="w-6 h-6 md:w-7 md:h-7" fill="currentColor" viewBox="0 0 24 24"><path d="M6 4h4v16H6V4zm8 0h4v16h-4V4z" /></svg>
                ) : (
                  <svg className="w-6 h-6 md:w-7 md:h-7" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
                )}
              </button>
              <span className="text-white text-[11px] md:text-sm font-mono tracking-wider">
                {formatTime(currentTime)} / {formatTime(duration)}
              </span>
            </div>
            <button onClick={toggleMute} className="text-white hover:text-[#FFD700] hover:scale-110 transition-all p-1">
              {isMuted ? (
                <svg className="w-6 h-6 md:w-7 md:h-7" fill="currentColor" viewBox="0 0 24 24"><path d="M16.5 12c0-1.77-1.02-3.29-2.5-4.03v2.21l2.45 2.45c.03-.2.05-.41.05-.63zm2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51C20.63 14.91 21 13.5 21 12c0-4.28-2.99-7.86-7-8.77v2.06c2.89.86 5 3.54 5 6.71zM4.27 3L3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.18v2.06c1.38-.31 2.63-.95 3.69-1.81L19.73 21 21 19.73l-9-9L4.27 3zM12 4L9.91 6.09 12 8.18V4z" /></svg>
              ) : (
                <svg className="w-6 h-6 md:w-7 md:h-7" fill="currentColor" viewBox="0 0 24 24"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z" /></svg>
              )}
            </button>
          </div>
        </div>
...
function VideoCard({ video, isDark, isLiked, isActive, onSelect, onLike, t, index }) {
  ...
  return (
    <motion.div
      ...
      className={`group relative rounded-2xl overflow-hidden cursor-pointer ${cardBg} border backdrop-blur-sm transition-all duration-500 hover:shadow-2xl hover:-translate-y-1 ${
    ...
    >
      <div className="relative aspect-video bg-black overflow-hidden">
        ...
        {duration > 0 && (
          <div className="absolute bottom-2 left-2 px-1.5 py-0.5 bg-black/85 text-white text-[10px] md:text-xs font-mono rounded z-20">
            {formatTime(duration)}
          </div>
        )}
        ...
        {isActive && (
          <div className={`absolute top-2 right-2 px-2 py-1 rounded-full ${accentBg} ${accentText} text-[10px] md:text-xs font-black flex items-center gap-1 z-20`}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-10 md:mb-14">
          <div className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full mb-5 ${badgeBg}`}>
            ...
            <span className={`text-[10px] md:text-xs font-black ${accentColorText} font-['Vazirmatn'] uppercase tracking-widest`}>
              {t('homePage.videoGallery.badge') || 'سرویس ویژه منزل'}
            </span>
          </div>
          <h2 className={`font-['Vazirmatn'] text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-black ${titleColor} tracking-tight leading-[1.05] transition-colors duration-300`}>
            {t('homePage.videoGallery.title') || 'تجربه‌ای سینمایی از سرویس کباب داغ ما'}
          </h2>
          <p className={`font-['Vazirmatn'] text-base sm:text-lg md:text-xl ${subtitleColor} mt-4 max-w-2xl mx-auto font-light leading-relaxed transition-colors duration-300`}>
            {t('homePage.videoGallery.subtitle') || 'نگاهی عمیق به نحوه ارائه خدمات، تجهیزات و لحظات خاص مهمانی‌ها'}
          </p>
        </motion.div>
